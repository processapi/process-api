...
Deno.test("ComponentModule.load_html - should return HTML content when url is provided", async () => {
    ...
    const html = await ComponentModule.load_html({ url: "component.js" });
...
Deno.test("ComponentModule.load_html - should include CSS link when hasCss is true", async () => {
    ...
    const html = await ComponentModule.load_html({ url: "component.js", hasCss: true });
    assertEquals(html, '<link rel="stylesheet" href="component.css"><div>Component HTML</div>');
});
